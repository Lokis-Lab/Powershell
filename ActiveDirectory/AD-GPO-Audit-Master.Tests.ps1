Describe 'AD-GPO-Audit-Master lazy AD dependency handling' {
  BeforeAll {
  ...
  It 'opens the two-GPO compare picker without forcing the Active Directory module' {
    ...
    $commandNames | Should -Not -Contain 'Ensure-AuditAdContextInitialized'
    $commandNames | Should -Not -Contain 'Initialize-GpoAuditAdContext'
  }

  It 'does not eagerly initialize AD before GPO-only CLI modes' {
    $functionAst = $script:Ast.Find({
      param($node)
      $node -is [System.Management.Automation.Language.FunctionDefinitionAst] -and
    ...
    $functionAst | Should -Not -BeNullOrEmpty
    $functionAst.Extent.Text | Should -Not -Match 'Import-GroupPolicyModule\s+Ensure-AuditAdContextInitialized'
  }
  ...
  It 'records an explicit GUI domain before showing the form without initializing AD' {
    ...
    $commandNames[0] | Should -Be 'Set-AuditRequestedDomain'
    $commandNames[1] | Should -Be 'Show-AdGpoAuditMasterMainGui'
    $commandNames | Should -Not -Contain 'Initialize-AuditAdContext'
    $commandNames | Should -Not -Contain 'Ensure-AuditAdContextInitialized'
  }
}
