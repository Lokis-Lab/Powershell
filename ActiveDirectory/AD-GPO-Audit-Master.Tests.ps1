...
Describe 'AD-GPO-Audit-Master offline GPO mode handling' {
  BeforeAll {
    $tokens = $null
    $parseErrors = $null
  ...
  It 'does not force Active Directory initialization for local-only GPO modes' {
    ...
    $commandNames | Should -Contain 'Import-GroupPolicyModule'
    ($commandNames | Where-Object { $_ -eq 'Ensure-AuditAdContextInitialized' }).Count | Should -Be 1
